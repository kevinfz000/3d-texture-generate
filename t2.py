...
def scan_training_data_with_texture(obj_folder, mtl_folder, tga_folder=None):
    ...
    for obj_file in obj_files:
        ...
        mtl_file = obj_basename + '.mtl'  # 默认MTL文件名
        mtl_path = os.path.join(mtl_folder, mtl_file)  # 默认MTL路径

        try:
            # 解析OBJ文件找到实际引用的MTL文件名
            with open(obj_path, 'r') as f:
                obj_content = f.read()
            mtl_ref = re.search(r'mtllib\s+(.+)\.mtl', obj_content)
            if mtl_ref:
                mtl_file = mtl_ref.group(1) + '.mtl'
...
class UpBlock(nn.Module):
    def __init__(self, in_channels, out_channels, skip_channels=None, use_attention=False):
        ...
        if skip_channels is None:
            conv_in = out_channels  # 没有跳跃连接时
        else:
            conv_in = out_channels + skip_channels  # 有跳跃连接时
        ...
        if use_attention:
            # 将 SelfAttention 改为 EfficientSelfAttention
...
class EnhancedTexturePredictor(nn.Module):
        def __init__(self, input_dim=33, mtl_output_dim=3, texture_size=256):
            ...
            self.decoder = nn.ModuleList([
                UpBlock(256, 192, skip_channels=192),  # 第一层跳跃连接192通道
                UpBlock(192, 128, skip_channels=256),  # 第二层跳跃连接256通道
                UpBlock(128, 96, skip_channels=None),  # 第三层没有跳跃连接
                UpBlock(96, 64, skip_channels=None),
                UpBlock(64, 48, skip_channels=None),
                UpBlock(48, 32, skip_channels=None, use_attention=True)
            ])
...
def train_model_with_texture(data_triplets, epochs=200, lr=0.0002, batch_size=32):
    ...
    # 优化器
    g_optimizer = torch.optim.Adam(generator.parameters(), lr=lr, betas=(0.92, 0.999))
    d_optimizer = torch.optim.Adam(discriminator.parameters(), lr=lr * 0.5, betas=(0.92, 0.999))
