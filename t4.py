...
def scan_training_data_with_texture(obj_folder, mtl_folder, tga_folder=None):
    ...
    for obj_file in obj_files:
        ...
        # 尝试解析OBJ文件找到引用的MTL文件名
        mtl_ref_name = None
        try:
            with open(obj_path, 'r') as f:
                for line in f:
                    if line.startswith('mtllib'):
                        mtl_ref_name = line.split()[1].strip()
                        break
        except Exception as e:
            print(f"Error reading OBJ {obj_file}: {e}")

        # 优先使用OBJ中引用的MTL文件名，否则使用默认命名
        if mtl_ref_name and os.path.exists(os.path.join(mtl_folder, mtl_ref_name)):
            mtl_file = mtl_ref_name
        else:
        ...
        if os.path.exists(mtl_path):
            ...
            print(f"Warning: No corresponding MTL found for {obj_file} at {mtl_path}")
...
def extract_mesh_features(verts, faces):
    ...
    try:
        ...
        _, normals = sample_points_from_meshes(
            mesh_torch,
            return_normals=True,
            num_samples=min(10000, len(verts))
        )
        ...
        verts = torch.clamp(verts, -1e5, 1e5)  # 防止极端值
        ...
        features = torch.clamp(features, -3.0, 3.0)  # 限制在[-3,3]区间
        return features
    except Exception as e:
    ...
    if mtl_path and os.path.exists(mtl_path):
        try:
            with open(mtl_path, 'r', encoding='utf-8', errors='ignore') as f:
                mtl_content = f.read()
            kd_match = re.search(r'Kd\s+([\deE+-]+)\s+([\deE+-]+)\s+([\deE+-]+)', mtl_content)
            if kd_match:
    ...
    if tga_folder:
        ...
        if os.path.exists(tga_path):
            try:
                ...
                traceback.print_exc()
                target_texture = torch.zeros(3, 512, 512)
        else:
...
class MeshDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if random.random() > 0.3:
                ...
                target_texture = target_texture[perm]

        if random.random() > 0.5:
                # 随机噪声注入
                noise = torch.randn_like(target_texture) * 0.02
                target_texture = torch.clamp(target_texture + noise, 0, 1)
...
class EfficientSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # 空间尺寸降低
        reduced_h = height // self.reduction
        reduced_w = width // self.reduction
        ...
        # 降低空间分辨率
        query = F.interpolate(query, size=(reduced_h, reduced_w), mode='bilinear')
        key = F.interpolate(key, size=(reduced_h, reduced_w), mode='bilinear')
        value = F.interpolate(value, size=(reduced_h, reduced_w), mode='bilinear')

        # 重塑形状
        query = query.view(batch_size, -1, reduced_h * reduced_w).permute(0, 2, 1)
        key = key.view(batch_size, -1, reduced_h * reduced_w)
        value = value.view(batch_size, -1, reduced_h * reduced_w)
        ...
        out = out.view(batch_size, -1, reduced_h, reduced_w)
...
class UpBlock(nn.Module):
    def __init__(self, in_channels, out_channels, skip_channels=None, use_attention=False):
        super().__init__()
        ...
        conv_in = out_channels + (skip_channels if skip_channels else 0)
    ...
    def forward(self, x, skip_connection=None):
        x = self.up(x)
        if skip_connection is not None and self.skip_channels:
            # 对齐空间尺寸
            if skip_connection.shape[2:] != x.shape[2:]:
                skip_connection = F.interpolate(
                    skip_connection, size=x.shape[2:],
...
class EnhancedTexturePredictor(nn.Module):
    def __init__(self, input_dim=33, mtl_output_dim=3, texture_size=256):
        ...
        self.encoder = nn.ModuleList([
            ResidualBlock(256),
            nn.Sequential(
                nn.Conv2d(256, 192, 4, stride=2, padding=1),
                nn.BatchNorm2d(192),
                nn.LeakyReLU(0.1, inplace=True),
                ResidualBlock(192))
        ])
        ...
        self.decoder = nn.ModuleList([
            UpBlock(256, 192, skip_channels=192),
            UpBlock(192, 128, skip_channels=256),
            UpBlock(128, 96),
            UpBlock(96, 64),
            UpBlock(64, 48),
            UpBlock(48, 32),  # 新增的上采样层
            UpBlock(32, 24),  # 新增的上采样层
            nn.Sequential(
                UpBlock(24, 16, use_attention=True),  # 注意调整输入通道数
                nn.Dropout2d(0.2)
            )
        ])
        ...
        self.final = nn.Sequential(
            nn.Conv2d(16, 32, 3, padding=1),  # 调整输入通道数
            nn.LeakyReLU(0.1, inplace=True),
    ...
    def forward(self, x):
        ...
        for i, module in enumerate(self.decoder):
            if isinstance(module, UpBlock):
                ...
                current = module(current, skip)
            else:
                # 对于非UpBlock模块（如Sequential），直接传递
                current = module(current)

        texture_pred = self.final(current)
        # 修改最终生成的纹理尺寸
        texture_pred = (texture_pred + 1) / 2  # [-1,1] -> [0,1]
        texture_pred = F.interpolate(
            texture_pred.unsqueeze(0),
            size=(512, 512),  # 改为目标分辨率
            mode='bilinear',
            align_corners=False
        ).squeeze(0)
...
def train_model_with_texture(data_triplets, epochs=200, lr=0.0002, batch_size=32):
    ...
    g_optimizer = torch.optim.Adam(
        ...
        lr=lr,
        betas=(0.88, 0.999),
        weight_decay=1e-5  # 新增L2正则化
    )
    d_optimizer = torch.optim.Adam(discriminator.parameters(), lr=lr * 0.5, betas=(0.9, 0.999))

    # 学习率调度器
    g_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        g_optimizer, T_max=50, eta_min=1e-6
    )
    ...
    for epoch in range(epochs):
        ...
        try:
            for batch_idx, batch in enumerate(dataloader):
                ...
                # 真实和假标签
                real_labels = torch.ones(batch_size) * 0.9  # 不是1.0
                fake_labels = torch.zeros(batch_size) + 0.1  # 不是0.0
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                    'generator': generator.module.state_dict(),  # 注意使用 .module
...
"""
        ...
        # 5. 更新OBJ文件 - 修复的关键部分
        with open(output_obj_path, 'r', encoding='utf-8', errors='ignore') as f:
        ...
        print(f"Error generating files for {obj_filename}: {str(e)}")
        traceback.print_exc()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Texture Generation Training Script')
    parser.add_argument('--local_rank', type=int, default=0, help='Local rank for distributed training')
    parser.add_argument('--training_obj_folder', type=str, required=True, help='Path to training OBJ folder')
    parser.add_argument('--training_mtl_folder', type=str, required=True, help='Path to training MTL folder')
    parser.add_argument('--training_tga_folder', type=str, required=True, help='Path to training TGA folder')
    parser.add_argument('--new_obj_folder', type=str, required=True, help='Path to new OBJ folder for generation')
    ...
    # ========== 第一步：准备训练数据 =========
    training_obj_folder = args.training_obj_folder  # 训练用OBJ文件夹
    training_mtl_folder = args.training_mtl_folder  # 训练用MTL文件夹
    training_tga_folder = args.training_tga_folder  # 训练用TGA文件夹
    ...
    with torch.no_grad():
        ...
        new_obj_folder = args.new_obj_folder  # 新OBJ文件夹
        output_folder = args.output_folder  # 输出文件夹
