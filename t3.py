    ...
    if mtl_path and os.path.exists(mtl_path):
        try:
            with open(mtl_path, 'r', encoding='utf-8', errors='ignore') as f:
                mtl_content = f.read()
            kd_match = re.search(r'Kd\s+([\d\.eE+-]+)\s+([\d\.eE+-]+)\s+([\d\.eE+-]+)', mtl_content)
            if kd_match:
...
class ResidualBlock(nn.Module):
    def __init__(self, features):
        ...
        self.block = nn.Sequential(
            spectral_norm(nn.Conv2d(features, features, 3, padding=1)),
...
class EnhancedTexturePredictor(nn.Module):
    def __init__(self, input_dim=33, mtl_output_dim=3, texture_size=256):
        ...
        self.decoder = nn.ModuleList([
            # 第一层：使用192通道跳跃连接（编码器第二层）
            UpBlock(256, 192, skip_channels=192),

            # 第二层：使用256通道跳跃连接（编码器第一层）
            ...
            # 最终层带注意力
            nn.Sequential(
                UpBlock(48, 32, use_attention=True),
                nn.Dropout2d(0.2)
            )
        ])
